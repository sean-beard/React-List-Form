...
function _tryTrustCertificate(certificatePath, parentTask) {
    switch (process.platform) {
        case 'win32':
            var certutilExePath = _ensureCertUtilExePath(parentTask);
            if (!certutilExePath) {
...
function ensureCertificate(canGenerateNewCertificate, parentTask) {
    ...
    if (certificateStore.certificateData && certificateStore.keyData) {
        if (!_certificateHasSubjectAltName(certificateStore.certificateData)) {
            var warningMessage = 'The existing development certificate is missing the subjectAltName ' +
...
function _ensureCertificateInternal(parentTask) {
    var certificateStore = CertificateStore_1.default.instance;
    var generatedCertificate = _createDevelopmentCertificate();
    var now = new Date();
