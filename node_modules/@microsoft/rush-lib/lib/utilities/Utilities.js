...
var Utilities = (function () {
    ...
    Utilities.executeCommandAsync = function (command, args, workingDirectory, environmentVariables) {
        ...
        var escapedCommand = Utilities.escapeShellParameter(command);
        var escapedArgs = args.map(function (x) { return Utilities.escapeShellParameter(x); });
     ...
     * @alpha
     */
    Utilities.executeShellCommand = function (command, workingDirectory, environmentVariables, captureOutput) {
        if (captureOutput === void 0) { captureOutput = false; }
        var shellCommand = process.env.comspec || 'cmd';
        var commandFlags = '/d /s /c';
        var useShell = true;
        ...
        var result = child_process.spawnSync(shellCommand, [commandFlags, command], {
            cwd: workingDirectory,
            shell: useShell,
            env: environmentVariables,
     ...
     * @alpha
     */
    Utilities.executeShellCommandAsync = function (command, workingDirectory, environmentVariables, captureOutput) {
        if (captureOutput === void 0) { captureOutput = false; }
        var shellCommand = process.env.comspec || 'cmd';
        var commandFlags = '/d /s /c';
        var useShell = true;
     ...
     */
    Utilities.escapeShellParameter = function (parameter) {
        return '"' + parameter + '"';
    };
    /**
     * Executes the command with the specified command-line parameters, and waits for it to complete.
    ...
    Utilities._executeCommandInternal = function (command, args, workingDirectory, stdio, environmentVariables) {
        ...
        var escapedCommand = command.indexOf(' ') < 0 ? command
            : Utilities.escapeShellParameter(command);
        var escapedArgs = args.map(function (x) { return Utilities.escapeShellParameter(x); });
        var result = child_process.spawnSync(escapedCommand, escapedArgs, options);
    ...
    Utilities._processResult = function (result) {
        if (result.error) {
