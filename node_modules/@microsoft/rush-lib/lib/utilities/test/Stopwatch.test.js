...
describe('Stopwatch', function () {
    ...
    it('gives 0.00 seconds after being reset', function (done) {
        ...
        watch.reset();
        chai_1.assert.equal(watch.toString(), '0.00 seconds (stopped)');
        chai_1.assert.equal(watch.duration, 0);
        done();
    });
    ...
    it('returns duration when the clock is stopped', function () {
        var watch = new Stopwatch_1.Stopwatch(pseudoTimeSeconds([0, 61.25]));
        watch.start();
        watch.stop();
        chai_1.assert.equal(watch.duration, 61.25);
    });
    it('returns duration using the latest time when the clock is not stopped', function () {
        var watch = new Stopwatch_1.Stopwatch(pseudoTimeSeconds([0, 1, 2]));
        watch.start();
        chai_1.assert.equal(watch.duration, 1);
        chai_1.assert.equal(watch.duration, 2);
    });
});
