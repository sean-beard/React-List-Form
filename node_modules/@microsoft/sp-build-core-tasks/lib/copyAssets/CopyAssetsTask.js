...
var CopyAssetsTask = (function (_super) {
    ...
    function CopyAssetsTask() {
        return _super.call(this, 'copy-assets', {
    ...
    CopyAssetsTask.prototype.executeTask = function (gulp, completeCallback) {
        ...
        if (shouldPrepareProductionDeployment) {
            ...
            var _loop_1 = function (webpackAsset) {
                ...
                if (notIgnoredExt && notEntryChunk) {
                    ...
                    if (!this_1.taskConfig.excludeHashFromFileNames) {
        ...
        bundleEntries.forEach(function (bundleEntry) {
            ...
            var manifestIds = [];
            Object.keys(bundleEntry.components).forEach(function (id) {
                var manifest = bundleEntry.components[id].manifestData;
                if (manifest) {
                    manifestIds.push(manifest.id);
                }
            });
            if (manifestIds.length === 0) {
                _this.logWarning("Missing manifest for entry \"" + bundleEntry.components[0].entrypoint + "\". " +
                    'Unable to prepare deployment for this entry.');
                return;
            }
            var stringsBaseName = manifestIds.length === 1 ? manifestIds[0] : FileUtils_1.getFullHash(manifestIds.join(''));
            var stringsModuleId = stringsBaseName + "_strings";
            _this._processEntry(gulp, deployEntry, shouldPrepareProductionDeployment, stringsModuleId, tasks);
            var entryWebpackChunk = webpackEntryChunks.get(bundleEntry.bundleName);
    ...
    CopyAssetsTask.prototype._copyStrings = function (gulp, stringsCopyCallbackMap, tasks) {
        var _this = this;
        ...
        stringsCopyCallbackMap.forEach(function (callbacks, key) {
            ...
            var _loop_2 = function (locale) {
                if (localizedFiles.hasOwnProperty(locale)) {
                    var filenameCallback = function (filename) { return callbacks.forEach(function (callback) { return callback(locale, filename); }); };
                    var gulpStream = _this.taskConfig.excludeHashFromFileNames ?
                        FileUtils_1.rename(gulp.src(localizedFiles[locale]), function (hash) { return baseFilename + "_" + locale + ".js"; }, filenameCallback) :
                        FileUtils_1.renameWithHash(gulp.src(localizedFiles[locale]), function (hash) { return baseFilename + "_" + locale + "_" + hash + ".js"; }, filenameCallback);
                    tasks.push(gulpStream.pipe(gulp.dest(_this.taskConfig.deployCdnPath)));
                }
            };
    ...
    CopyAssetsTask.prototype._processEntry = function (gulp, deployEntry, shouldPrepareProductionDeployment, stringsModuleId, tasks) {
        var _this = this;
        ...
        var getFilenameWithHashCallback;
        ...
        if (bundleEntry.nonLocalizedEntrypointPath) {
            ...
            if (shouldPrepareProductionDeployment) {
                ...
                getFilenameWithHashCallback = function (hash) { return entryName.toLowerCase() + "_" + hash + ".js"; };
                getFilenameCallback = function () { return entryName.toLowerCase() + ".js"; };
            }
        }
            ...
            for (var locale in bundleEntry.localizedEntrypointPaths) {
                if (bundleEntry.localizedEntrypointPaths.hasOwnProperty(locale)) {
                    var entrypointPath = bundleEntry.localizedEntrypointPaths[locale];
                    reverseEntrypointLookup_1[entrypointPath] = locale;
            ...
            if (shouldPrepareProductionDeployment) {
                ...
                getFilenameWithHashCallback = function (hash, originalFilename) {
                    var locale = reverseEntrypointLookup_1[originalFilename];
                    if (_this.taskConfig.localeInDirectoryName) {
                ...
                getFilenameCallback = function (originalFilename) {
                    var locale = reverseEntrypointLookup_1[originalFilename];
                    if (_this.taskConfig.localeInDirectoryName) {
    ...
    CopyAssetsTask.prototype._transformBundleHeader = function (entryGulpStream, bundleEntry) {
        var escapedName = lodash_1.escapeRegExp(this.properties.webpackLibraryName);
        if (Object.keys(bundleEntry.components).length > 1) {
            ...
            var bundleTemporaryNameRegex = new RegExp("^(define\\()(\\\"|\\')" + escapedName + "\\2,\\s*");
            ...
            // If we change the type of bundle we're producing, we need to update this
            var bundleTemporaryNameRegex = new RegExp("^(define\\((\\\"|\\'))(?:" + escapedName + ")((?:\\2),)");
            var correctName = ConfigureWebpackTask_1.getBundleId(bundleEntry);
    ...
    CopyAssetsTask.prototype._processExternals = function (gulp, externalModules, tasks) {
        var _this = this;
        externalModules.forEach(function (externalModule) {
            if (externalModule.renameCallbacks.length > 0) {
                ...
                var fileCallback = function (filename) { return externalModule.renameCallbacks.forEach(function (renameCallback) { return renameCallback(filename); }); };
                var gulpStream = _this.taskConfig.excludeHashFromFileNames ?
                    FileUtils_1.rename(gulp.src(externalModule.configExternalFullPath), function () { return baseFilename_1.toLowerCase() + ".js"; }, fileCallback) :
                    FileUtils_1.renameWithHash(gulp.src(externalModule.configExternalFullPath), function (hash) { return baseFilename_1.toLowerCase() + "_" + hash + ".js"; }, fileCallback);
                tasks.push(gulpStream.pipe(gulp.dest(_this.taskConfig.deployCdnPath)));
            }
        });
    };
    ...
    CopyAssetsTask.prototype._fillLocalizedPaths = function (paths, defaultLocale, moduleConfig) {
        for (var locale in paths) {
            if (paths.hasOwnProperty(locale)) {
                var localizedPath = paths[locale];
                this._fillLocalizedPath(localizedPath, locale, defaultLocale, moduleConfig);
            }
        }
    };
    CopyAssetsTask.prototype._fillLocalizedPath = function (localizedPath, locale, defaultLocale, moduleConfig) {
