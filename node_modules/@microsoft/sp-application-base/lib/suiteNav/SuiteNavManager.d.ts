...
/**
 * The SuiteNavManager interface manages the loading and visibility of suite nav
 *
 * @alpha
 */
export interface ISuiteNavManager extends IComponentVisible {
    /**
     * Initiates the loading of suite nav. By default, loading suite nav is handled automatically by the framework.
     * This function is only needed if an application explictly have the suite nav disabled initially.
     */
    loadSuiteNav(suiteNavConfig: ISuiteNavManagerConfiguration): void;
}
export default class SuiteNavManager implements ISuiteNavManager {
    ...
    private _suiteNavModelPromise;
    private _serviceScope;
    private static _updateCache(model, service);
    private static _clearCache();
    constructor(suiteNavDiv: HTMLDivElement, serviceScope: ServiceScope);
    /**
     ...
     */
    private _getSuiteNavModel(forceFetchData?);
     ...
     */
    private _getSuiteNavShell(cssUrl, jsUrl);
    /**
     ...
     */
    private _renderSuiteNav(o365Shell, navBarData);
